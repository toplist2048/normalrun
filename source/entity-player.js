...
class entity_player_t extends entity_t {
	...
	_update() {
		...
		var angle = Math.atan2(
			...
			mouse_x - (t.x + 6 + _gl.camera_x + c.width * 0.5)
		);
		t.s = 18 + ((angle / Math.PI * 4 + 10.5) % 8)|0;

		// bobbing
		t._bob += time_elapsed * 1.75 * (Math.abs(t.vx) + Math.abs(t.vz));
		...
		if (keys[key_shoot] && t._last_shot < 0) {
			...
			new entity_plasma_t(t.x, 0, t.z, 0, 26, angle + Math.random() * 0.2 - 0.11);
			t._last_shot = 0.1;
		}
	...
	_render() {
		...
		_gl.push_light(this.x, 4, this.z + 6, 1, 0.5, 0, 0.04);
	}
